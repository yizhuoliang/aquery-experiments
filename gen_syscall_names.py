...
# Extract syscall name and its number
matches = re.findall(r'NACL_sys_(\w+)\s+(\d+)', definitions)

with open('syscall_array.txt', 'w') as f:
    f.write("const char *syscall_names[] = {\n")

    for match in matches:
        syscall_name, syscall_num = match
        f.write(f"    [{syscall_num}] = \"{syscall_name}\",\n")

    f.write("};\n")

print("Array definition written to syscall_array.txt!")
